...
private struct PackOpenSpotlightOverlay: View {
    ...
    var body: some View {
        GeometryReader { _ in
            let holeShape = RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
            let holeFrame = CGSize(width: max(0, rect.width), height: max(0, rect.height))
            ...
            ZStack {
                ...
                holeShape
                    .fill(Color.black)
                    .frame(width: holeFrame.width, height: holeFrame.height)
