...
struct StudioConsoleView: View {
    ...
    fileprivate struct SettingsLissajousPreview: View {
        let e3: Color
        ...
        var body: some View {
            TimelineView(.animation) { tl in
                ...
                let (rx, ry) = pickPair(for: step)

                // Slow phase drift so it feels “alive” while staying stable/readable.
                ...
                Canvas { ctx, size in
                    ...
                    // Global opacity (respects your accessibility reductions upstream)
                    ctx.opacity = max(0, min(1, globalAlpha))

                    // Background (very light “screen” so the curve reads on both schemes)
                    ctx.fill(Path(roundedRect: rect, cornerRadius: 12), with: .color(Color.secondary.opacity(0.06)))
                    ...
                    // Ensure closure: sample across an integer multiple of both denominators.
                    let period = Double(StudioConsoleView.tenneyLCM(rx.d, ry.d))
                    
                    let n = max(64, min(8192, samples))
                    var curve = Path()

                    func point(_ i: Int) -> CGPoint {
                        let u = Double(i) / Double(max(1, n - 1))      // 0…1
                        let tt = u * period
                    ...
                    if dotMode {
                        ...
                        let stepDots = max(1, n / 600) // cap dot count for perf
                    ...
                    // Tiny label (optional but helps “what am I seeing?”)
